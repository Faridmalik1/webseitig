...
export function ChatWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
      {isOpen && (
        <div className="w-[320px] rounded-3xl border border-slate-700 bg-slate-800 shadow-2xl">
          {/* Header */}
          ...
          <div className="rounded-b-3xl border-t border-slate-700 bg-slate-800 px-4 py-3">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-full border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 outline-none focus:border-indigo-500"
                placeholder="Ask a question..."
              />
              <button
                onClick={sendQuestion}
                disabled={!canSend || loading}
                className="rounded-full bg-[#C8F135] px-4 py-2 text-sm text-black transition hover:bg-[#B8E125] disabled:cursor-not-allowed disabled:opacity-50"
              >
                Send
              </button>
            </div>
