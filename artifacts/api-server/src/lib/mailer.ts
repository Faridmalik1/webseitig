...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    strong { color:#fff; }
    .cta { background:#C8E646; color:#171717; font-weight:700; font-size:14px; padding:10px 24px; border-radius:50px; display:inline-block; text-decoration:none; margin:8px 0 0; }
    .footer { background:#141414; padding:16px 32px; text-align:center; font-size:12px; color:rgba(255,255,255,0.25); border-top:1px solid rgba(255,255,255,0.05); }
  </style>
</head>
<body>
...
export async function sendContactEmails(data: ContactFormData): Promise<void> {
  ...
  if (!transport || !adminEmail) {
    logger.info({ data }, "SMTP not configured — contact email not sent (mock mode)");
    return;
  }
...
export async function sendLeadEmails(data: LeadFormData): Promise<void> {
  ...
  if (!transport || !adminEmail) {
    logger.info({ data }, "SMTP not configured — lead email not sent (mock mode)");
    return;
  }
  ...
  logger.info({ name: data.name, adminEmail }, "Lead emails sent");
}
